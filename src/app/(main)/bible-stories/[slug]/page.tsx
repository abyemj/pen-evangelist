...
interface BibleStoryPageProps {
  params: { slug: string };
}

export async function generateStaticParams() {
  const posts = getAllPosts().filter(p => p.category === 'bible-story');
  return posts.map((post) => ({
    slug: post.slug,
  }));
}

export async function generateMetadata({ params }: BibleStoryPageProps) {
  const story = getPostBySlug(params.slug);
  if (!story || story.category !== 'bible-story') {
    return {
      title: 'Story Not Found',
    };
  }
  return {
    title: `${story.title} | Pen Evangelist`,
    description: story.excerpt,
  };
}

export default function BibleStoryPage({ params }: BibleStoryPageProps) {
  const story = getPostBySlug(params.slug);
      ...
      {story.imageUrl && (
        <div className="relative w-full h-72 md:h-96 rounded-lg overflow-hidden shadow-lg mb-8">
          <Image
            ...
            objectFit="cover"
            priority
            data-ai-hint={story.dataAiHint || 'scripture illustration'}
          />
        </div>
      )}
